...
const stateRoot = persistentStateRoot ?? await mkdtemp(join(tmpdir(), 'paperclip-clippy-plugin-manual-'));
const paperclipHome = join(stateRoot, 'paperclip-home');
const dataDir = join(stateRoot, 'paperclip-data');
const instanceId = 'paperclip-clippy-plugin-manual';
const pluginInstallKey = 'paperclip-clippy-plugin';
const startPath = '/instance/settings/plugins';
...
const shouldOpenBrowser = process.env.PAPERCLIP_E2E_OPEN_BROWSER !== 'false';
const env = {
  ...process.env,
  CI: 'true',
  BROWSER: 'none',
...
function log(message) {
  console.log(`[paperclip-clippy-plugin:manual] ${message}`);
}
...
async function ensureCompanySeeded() {
  ...
  const createdCompany = await fetchJson(companiesUrl, {
    ...
    body: JSON.stringify({
      name: 'Dummy Company',
      description: 'Seed company for manual paperclip-clippy-plugin verification.'
    })
  });
...
async function ensurePluginInstalled(configPath) {
  try {
    ...
    log('Installed local paperclip-clippy-plugin plugin.');
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    if (message.includes(`Plugin already installed: ${pluginInstallKey}`)) {
...
async function maybeOpenUrl(url) {
  if (!shouldOpenBrowser) {
    log('Browser auto-open disabled by PAPERCLIP_E2E_OPEN_BROWSER=false.');
    return;
  }

  try {
    await runCommand('open', [url], { stdio: 'ignore' });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    log(`Could not auto-open ${url}: ${message}`);
  }
}
...
async function main() {
  ...
  console.log('1. Confirm there is no custom Clippy settings content or test-trigger UI in the plugin manager.');
  console.log('2. Confirm there is no visible Clippy button in the global toolbar.');
  console.log('3. Trigger a few normal Paperclip actions that show host toasts.');
  console.log('4. Confirm the native toast is hidden or only flashes briefly.');
  console.log('5. Confirm the Clippy bubble shows intercepted toast text automatically and queues multiple messages in order.');
  console.log('');
  console.log('Press Ctrl+C when you are done. Disposable state will be cleaned up automatically.');
